...
# --------------------------------------------------------------------------- #
# Platform adapter (docs/ARCHITECTURE.md D5): the wave tree is a CONTEXT
# provider — its float confidence feeds signal confidence as one component and
# reporting shows its alternates; it never gates a trade by itself.
# --------------------------------------------------------------------------- #
def tree_context(bars, window: int = 400) -> dict:
    """Ranked top-down counts summarized for the signal engine / reports.

    Returns {"tree_confidence": float|None, "primary": str|None,
             "alternates": [str, ...]} — degrade gracefully on thin data."""
    try:
        counts = wave_counts(bars[-window:] if window else bars)
    except Exception:
        counts = []
    if not counts:
        return {"tree_confidence": None, "primary": None, "alternates": []}
    prim = counts[0]
    def _desc(ac):
        return f"{ac.pattern} @ {ac.degree_label} (conf {ac.confidence:.2f})"
    return {"tree_confidence": round(prim.confidence, 4),
            "primary": _desc(prim),
            "alternates": [_desc(a) for a in counts[1:3]]}
